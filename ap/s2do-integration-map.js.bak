...
const S2DOIntegrationMap = () => {
    ...
    const getPhaseColor = (phaseId) => {
        var _a;
        return ((_a = phases.find(phase => phase.id === phaseId)) === null || _a === void 0 ? void 0 : _a.color) || '#333';
    };
    ...
    return (<div className="p-4 max-w-6xl mx-auto bg-gray-50 rounded-lg shadow-lg">
      ...
      <div className="mb-6 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap gap-3 justify-center">
          {phases.map(phase => (<div key={phase.id} className="flex items-center gap-2">
      ...
      <div className="relative bg-white p-4 rounded-lg border border-gray-200 shadow-sm mb-6" style={{ height: '500px' }}>
        ...
        <svg className="absolute inset-0 w-full h-full" style={{ zIndex: 1 }}>
          {integrationNodes.map(node => node.connections.map(targetId => {
            ...
            return (<line key={`${node.id}-${targetId}`} x1={`${sourceLeft}%`} y1={sourceTop} x2={`${targetLeft}%`} y2={targetTop} stroke={getPhaseColor(node.phaseOrigin)} strokeWidth="2" strokeOpacity="0.5" strokeDasharray={activeNode === node.id || activeNode === targetId ? "none" : "5,5"} style={{ transition: "all 0.3s ease" }}/>);
        }))}
        </svg>
        ...
        {integrationNodes.map(node => {
            ...
            return (<div key={node.id} className={`absolute p-2 rounded-lg shadow-md cursor-pointer transition-all duration-300 border-2 ${activeNode === node.id ? 'z-20 scale-110' : 'z-10'}`} style={{
                    left: position.left,
                    top: position.top,
              ...
              <div className="text-xs text-gray-600">
                {activeNode === node.id ? node.description : `From ${(_a = phases.find(p => p.id === node.phaseOrigin)) === null || _a === void 0 ? void 0 : _a.name} Phase`}
              </div>
            </div>);
        })}
      </div>
      ...
      <div className="bg-gray-100 p-4 rounded-lg border border-gray-300 mb-6">
        ...
        (_a = useAlternatives.find(a => a.capability === activeNode)) === null || _a === void 0 ? void 0 : _a.alternatives.map((alt, idx) => (<div key={idx} className="bg-white p-3 rounded shadow-sm border border-gray-200">
...
exports.default = S2DOIntegrationMap;
