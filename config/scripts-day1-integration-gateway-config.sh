...
CREDENTIALS_FILE="./secure-credentials.json"  # Path to your secured credentials file
...
find "$INTEGRATION_DIR" -name ".env" -type f -exec bash -c "update_env_file {}" \;
find "$INTEGRATION_DIR" -name ".env.production" -type f -exec bash -c "update_env_file {}" \;
find "$INTEGRATION_DIR" -name ".env.local" -type f -exec bash -c "update_env_file {}" \;
