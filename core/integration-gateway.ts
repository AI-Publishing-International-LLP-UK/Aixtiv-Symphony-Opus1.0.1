...
const CredentialManager = {
  ...
  async _getSecret(secretName) {
    try {
      console.log(`Accessing secret from GCP Secret Manager: ${secretName}`);
      
      // Initialize the Secret Manager client
      const client = new SecretManagerServiceClient();
      
      // Format the secret name properly
      const projectId = process.env.GOOGLE_CLOUD_PROJECT || 'api-for-warp-drive';
      ...
      console.warn(`Failed to get secret from GCP Secret Manager: ${error.message}`);
      console.warn('Falling back to mock secrets for development environment');
      
      // For development environment, fall back to mock values
      const mockSecrets = {
