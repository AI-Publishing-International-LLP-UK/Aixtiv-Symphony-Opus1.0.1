...
class OpenAIAdapter extends ServiceAdapter {
  ...
  async generateCompletion(params, connection) {
    ...
    try {
      console.log(
        `Generating completion (${requestId}) with OpenAI (${this.serviceId})...`
      );
      ...
      while (retries < maxRetries) {
        try {
          ...
          const shouldRetry = this._shouldRetryRequest(
            error,
            retries,
            maxRetries
          );
          ...
          console.log(
            `Retrying OpenAI request (attempt ${retries}/${maxRetries}) after ${backoffTime}ms`
          );
      ...
      console.log(
        `OpenAI completion generated (${requestId}) in ${duration}ms`
      );
  ...
  async generateEmbedding(params, connection) {
    ...
    try {
      console.log(
        `Generating embedding (${requestId}) with OpenAI (${this.serviceId})...`
      );
  ...
  async preprocess(operation, params, options) {
    if (operation === 'generateCompletion') {
      ...
      if (
        !processedParams.max_tokens &&
        this.modelMapping[processedParams.model]
      ) {
        processedParams.max_tokens =
          this.modelMapping[processedParams.model].default_tokens;
      }

      // Handle prompt formatting for few-shot learning if examples are provided
      if (
        processedParams.examples &&
        Array.isArray(processedParams.examples) &&
        processedParams.examples.length > 0
      ) {
        ...
        formattedPrompt +=
          processedParams.finalPrompt || processedParams.prompt;
