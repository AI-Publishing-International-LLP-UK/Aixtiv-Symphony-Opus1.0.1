...
export class ProactiveAgent {
  ...
  async confirmCompletion(context){
    ...
    const verificationResult =
      await this.completionVerifier.verifyCompletion(context);

    // Generate base completion message
    let message = await this.createBaseCompletionMessage(
      context,
      verificationResult
    );
  ...
  async generateNextSteps(context){
    ...
    if (this.marketingInterface) {
      const enhancedSteps =
        await this.marketingInterface.enhanceNextSteps(nextSteps);
   ...
   * @*/
  async analyzeInitiativeOpportunity(
    context){ score; confidence: number }> {
   ...
   * @param context Current context
   * @*/
  async addWorkflowBoundaries(
    message,
    context){
    const boundaries =
      await this.boundaryManager.defineWorkflowBoundaries(context);
   ...
   * @*/
  async createBaseCompletionMessage(
    context,
    verificationResult){
   ...
   * @*/
  async createBaseNextStepsMessage(
    context,
    nextSteps){
    ...
    let message =
      "Based on what we've accomplished, here are the recommended next steps:\n\n";
...
export class BidOpportunityAgent extends ProactiveAgent {
  ...
  async addWorkflowBoundaries(
    ...
    let processSteps =
      'qualification assessment, competitive analysis, and bid strategy development';
    if (opportunities.length > 1) {
      processSteps =
  ...
  async createBaseCompletionMessage(
    ...
    if (opportunities.length === 1) {
      ...
      const recommendedCount =
        verificationResult.recommendedOpportunities?.length || 0;
  ...
  async createBaseNextStepsMessage(
    ...
    let message =
      'Based on the opportunity analysis, here are the recommended next steps:\n\n';
...
export class ContentOpportunityAgent extends ProactiveAgent {
  ...
  async analyzeInitiativeOpportunity(
    ...
    const avgValueScore =
      insights.reduce((sum, insight) => sum + insight.valueScore, 0) /
      insights.length;
    ...
    // Combine factors for final score
    const score =
      avgValueScore * 0.6 + quantityFactor * 0.2 + audienceMatchFactor * 0.2;
  ...
  async addWorkflowBoundaries(
    ...
    let processSteps =
      'outline creation, content development, and distribution planning';
    if (insights.length > 1) {
      processSteps =
  ...
  async createBaseCompletionMessage(
    ...
    if (contentItems.length === 1) {
      ...
      return `I've completed the development of "${content.title}" ${content.format}. The final piece includes ${content.wordCount} words, ${content.sections} sections, and is optimized for your ${content.targetAudience} audience. All quality checks have been passed with a quality score of ${content.qualityScore}/100.`;
    } else {
  ...
  async createBaseNextStepsMessage(
    ...
    let message =
      'Now that your content is ready, here are the recommended next steps to maximize its impact:\n\n';
...
export class VerificationAgent extends ProactiveAgent {
  ...
  async analyzeInitiativeOpportunity(
    ...
    const verificationOpportunities =
      await this.blockchainService.identifyVerificationOpportunities(
    ...
    const avgImportanceScore =
      verificationOpportunities.reduce(
  ...
  async createBaseInitiativeMessage(context){
    ...
    const urgentOpportunities = verificationOpportunities.filter(
      opp => opp.isUrgent
    );
  ...
  async addWorkflowBoundaries(
    ...
    const hasOwnership = opportunities.some(
      opp => opp.verificationType === 'ownership'
    );
    const hasAuthenticity = opportunities.some(
      opp => opp.verificationType === 'authenticity'
    );
    const hasCompliance = opportunities.some(
      opp => opp.verificationType === 'compliance'
    );

    let processSteps =
      'data preparation, blockchain submission, and verification confirmation';
  ...
  async createBaseNextStepsMessage(
    ...
    let message =
      'Based on the completed verifications, here are the recommended next steps:\n\n';
    ...
    if (
      verificationResult.failedItems &&
