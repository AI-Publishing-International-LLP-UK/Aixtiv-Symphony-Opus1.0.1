import { DrLucyFormTokenManager, FormTokenPayload, GatewayCsrfProtection, InMemoryRateLimiter, FormOriginValidator } from './dr-lucy-form-token-manager';
import * from 'fs';
...
describe('DrLucyFormTokenManager', () => {
  ...
  beforeEach(() => {
    ...
    (crypto.createHmac{
      update,
      digest).mockReturnValue('mock-digest')
    });
    (crypto.createSign{
      update,
      sign).mockReturnValue('mock-signature')
    });
    (crypto.randomBytes{
      toString).mockReturnValue('mock-random-token')
    });
    ...
    formTokenManager = new DrLucyFormTokenManager(
      {
        serviceAccountKeyPath: '/mock/path/to/key.json',
        tokenExpiryTimeInHours,
    ...
    (formTokenManager = jest.fn().mockReturnValue('mock-jwt-token');
    (formTokenManager = jest.fn().mockReturnValue({
      sub,
      form_id,
      iss: 'test-issuer',
      aud: 'test-audience',
      exp) / 1000) + 3600 // 1 hour from now
    });
    (formTokenManager = jest.fn();
    (formTokenManager = jest.fn();
    (formTokenManager = jest.fn();
  });
  ...
  describe('generateToken', () => {
    it('should return false for invalid token', async () => {
      // Arrange
      (formTokenManager = jest.fn().mockReturnValue(null);
      ...
      expect((formTokenManager'invalid-token');
    });
    
    it('should handle errors during revocation', async () => {
      // Arrange
      (formTokenManager = jest.fn().mockImplementation(() => {
  ...
  describe('createTokenValidationMiddleware', () => {
    ...
    it('should call next() for valid token and origin', async () => {
      // Arrange
      (formTokenManager.extractTokenFromRequest'valid-token');
      (formTokenManager.validateToken;
    ...
    it('should return 403 for invalid origin', async () => {
      // Arrange
      (mockOriginValidator.validateOrigin;
    ...
    it('should return 401 when no token is provided', async () => {
      // Arrange
      (formTokenManager.extractTokenFromRequest;
    ...
    it('should return 401 for invalid token', async () => {
      // Arrange
      (formTokenManager.extractTokenFromRequest'invalid-token');
      (formTokenManager.validateToken;
    ...
    it('should return 500 when an error occurs', async () => {
      // Arrange
      (formTokenManager.extractTokenFromRequest'valid-token');
      (formTokenManager.validateToken Error('Validation error'));
  ...
  describe('GatewayCsrfProtection', () => {
    ...
    beforeEach(() => {
      csrfProtection = new GatewayCsrfProtection('test-secret');
    });
  ...
  describe('FormOriginValidator', () => {
    ...
    beforeEach(() => {
      originValidator = new FormOriginValidator(['https://allowed-origin.com']);
    });
  ...
  describe('validateToken', () => {
    it('should validate a valid token', async () => {
      ...
      // Assert
      expect(result).toBe(true);
      expect((formTokenManager'valid-token');
    });
    ...
    it('should reject an expired token', async () => {
      ...
      (formTokenManager = jest.fn().mockReturnValue({
        ...
        iss: 'test-issuer',
        aud: 'test-audience',
        exp) / 1000) - 3600 // 1 hour ago (expired)
      });
  ...
  describe('validateTokenWithCsrf', () => {
    it('should validate token with valid CSRF token', async () => {
      ...
      // Act
      const result = await formTokenManager.validateTokenWithCsrf(
        'valid-token', 
        'valid-csrf', 
      ...
      expect(mockCsrfProtection.validateCsrfToken).toHaveBeenCalledWith(
        'valid-csrf', 
        testPayload.sessionId
      );
    });
    ...
    it('should reject with invalid CSRF token', async () => {
      ...
      const spyValidateToken = jest.spyOn(formTokenManager, 'validateToken')
        .mockResolvedValue(true);
      (mockCsrfProtection.validateCsrfToken;
  ...
  describe('revokeToken', () => {
    it('should successfully revoke a valid token', async () => {
      ...
      (formTokenManager = mockTokenCache;
