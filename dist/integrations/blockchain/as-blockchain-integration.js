...
export class BlockchainIntegrationManager {
  ...
  constructor(
    ...
    nftContractAddress= process.env.AIXTIV_NFT_CONTRACT_ADDRESS || '',
    verificationContractAddress= process.env
      .AIXTIV_VERIFICATION_CONTRACT_ADDRESS || '',
    subscriptionContractAddress= process.env
   ...
   */
  createUserWallet(){ address; privateKey: string } {
  ...
  async verifyUser(
    userAddress,
    userCode,
    userId){ transactionId; status: boolean }> {
    try {
      // Create hash of the user code
      const userCodeHash = ethers.utils.keccak256(
        ethers.utils.toUtf8Bytes(userCode)
      );

      // Call verification contract
      const tx = await this.verificationContract.verifyUser(
        userAddress,
        userCodeHash
      );
      ...
      // Get event data
      const verifiedEvent = receipt.events.find(
        (e=> e.event === 'UserVerified'
      );
  ...
  async verifyOrganization(
    ...
    try {
      ...
      // Get event data
      const verifiedEvent = receipt.events.find(
        (e=> e.event === 'OrganizationVerified'
      );
      const verificationStatus = verifiedEvent.args.status;
  ...
  async mintAgentNFT(
    ...
    agentId,
    agentType,
    metadata){ tokenId; transactionId: string }> {
    try {
  ...
  async mintSolutionNFT(
    ...
    try {
      ...
      // Mint the NFT
      const tx = await this.nftContract.mintSolution(
        ownerAddress,
        metadataUri,
        solutionCode
      );
  ...
  async transferNFT(
    tokenId,
    fromAddress,
    toAddress){ transactionId: string }> {
    try {
      // Transfer the NFT
  ...
  async createSubscription(
    ...
    durationInDays,
    subscriberId,
    solutionId){ subscriptionId; transactionId: string }> {
    try {
  ...
  async renewSubscription(
    subscriptionId,
    additionalDays){ transactionId; newEndTime: number }> {
    try {
      // Convert days to seconds
      ...
      const renewedEvent = receipt.events.find(
        (e=> e.event === 'SubscriptionRenewed'
      );
  ...
  async cancelSubscription(
    ...
    try {
      ...
      const tx =
        await this.subscriptionContract.cancelSubscription(subscriptionId);
      const receipt = await tx.wait();

      // Get the canceled time from the SubscriptionCanceled event
      const canceledEvent = receipt.events.find(
        (e=> e.event === 'SubscriptionCanceled'
      );
  ...
  async getSubscription(subscriptionId){
    try {
      const subscription =
        await this.subscriptionContract.getSubscription(subscriptionId);
  ...
  async getSubscriptionsBySubscriber(
    ...
    try {
      const subscriptionIds =
        await this.subscriptionContract.getSubscriptionsBySubscriber(
          subscriberAddress
        );
  ...
  async createVerificationRecord(
    ...
    try {
      ...
      const verificationData = {
        ...
        transactionId,
        timestamp,
        verificationHash, // Remove '0x' prefix
        verificationStatus,
      ...
      // Add to Firestore
      await setDoc(
        doc(db, 'blockchainRecords', verificationData.id),
        verificationData
      );
      ...
      if (recordType === 'user') {
        ...
        await updateDoc(doc(db, 'organizations', recordId), {
          ...
          'blockchainVerification.transactionId',
    ...
    try {
      ...
      if (nftData.tokenType === 'agent') {
        ...
        const { ownerType, ownerId } =
          await this.resolveAddressToOwner(newOwnerAddress);
  ...
  async createSubscriptionRecord(
    ...
    try {
      ...
      // Add to Firestore
      await setDoc(
        doc(db, 'subscriptions', subscriptionData.id),
        subscriptionData
      );
   ...
   */
  async resolveAddressToOwner(
    address){ ownerType; ownerId: string }> {
    try {
...
export class S2DOBlockchainSecurityManager {
  ...
  async verifyObjectHash(
    objectId,
    objectHash){ verified; blockchainRecord?: any }> {
    try {
  ...
  async registerObject(
    objectId,
    objectData,
    ownerAddress){ transactionId; objectHash: string }> {
    try {
      // Create object hash
