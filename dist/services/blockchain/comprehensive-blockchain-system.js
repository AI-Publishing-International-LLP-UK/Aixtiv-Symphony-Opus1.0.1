...
class BlockchainManager {
  ...
  connectWallet(privateKey) {
    ...
    this.contracts.flightVerification =
      this.contracts.flightVerification.connect(wallet);
    this.contracts.deliverableAuthorization =
      this.contracts.deliverableAuthorization.connect(wallet);
    this.contracts.rewardDistribution =
      this.contracts.rewardDistribution.connect(wallet);
...
class QRAuthSystem {
  constructor(blockchainManager) {
    ...
    this.secretKey =
      process.env.QR_AUTH_SECRET || 'aixtiv-symphony-default-key';
  }
  ...
  async processAuthorization(deliverableId, approved, ownerWallet) {
    try {
      ...
      const tx =
        await this.blockchain.contracts.deliverableAuthorization.recordAuthorization(
          deliverableId,
          approved,
          Date.now()
        );
  ...
  async _triggerRewards(deliverableId) {
    ...
    const qualityMultiplier = deliverableData.qualityScore
      ? deliverableData.qualityScore / 5
      : 1;
    ...
    const tx =
      await this.blockchain.contracts.rewardDistribution.distributeRewards(
        deliverableId,
        deliverableData.pilotId,
        pilotPoints,
        deliverableData.groundCrewIds || [],
        Array(deliverableData.groundCrewIds?.length || 0).fill(
          groundCrewPoints
        ),
    ...
    await db.collection('rewards').add({
      ...
      groundCrewPoints: Array(deliverableData.groundCrewIds?.length || 0).fill(
        groundCrewPoints
      ),
...
class FlightVerificationSystem {
  ...
  async recordCheckpoint(flightId, checkpointName, checkpointData) {
    try {
      ...
      const tx =
        await this.blockchain.contracts.flightVerification.recordCheckpoint(
          flightId,
          checkpointName,
          Date.now(),
  ...
  async completeFlight(flightId, deliverables, rating, notes) {
    try {
      ...
      const isPerfectFlight =
        durationMinutes >= 115 && durationMinutes <= 125 && rating === 5.0;
      ...
      const tx =
        await this.blockchain.contracts.flightVerification.completeFlight(
          flightId,
          Date.now(),
      ...
      if (isPerfectFlight) {
        await this._distributePerfectFlightRewards(
          flightId,
          flightData.pilotId
        );
      }
  ...
  async _distributePerfectFlightRewards(flightId, pilotId) {
    try {
      ...
      const tx =
        await this.blockchain.contracts.rewardDistribution.distributePerfectFlightRewards(
          flightId,
          pilotId,
...
class AgentRegistrySystem {
  ...
  async verifyAgentOnChain(agentId) {
    try {
      ...
      const agentData =
        await this.blockchain.contracts.agentRegistry.getAgent(agentId);
      ...
      const isMatch =
        ...
        JSON.stringify(blockchainCapabilities) ===
          JSON.stringify(firestoreData.capabilities) &&
...
blockchainApp.post('/api/nft/mint', async (req, res) => {
  try {
    ...
    const result = await giftShopNFTSystem.mintAgentNFT(
      userId,
      agentType,
      attributes
    );
...
blockchainApp.post('/api/nft/transfer', async (req, res) => {
  try {
    ...
    const result = await giftShopNFTSystem.transferAgentNFT(
      tokenId,
      fromUserId,
      toUserId
    );
...
exports.blockchainAPI = functions
  .region('us-west1')
  .https.onRequest(blockchainApp);

// Firestore triggers for blockchain sync
// Sync new deliverables to blockchain
exports.onNewDeliverable = functions
  .region('us-west1')
  ...
  .onCreate(async (snapshot, context) => {
    try {
      ...
      const tx =
        await blockchainManager.contracts.deliverableAuthorization.createDeliverable(
          deliverableId,
          deliverableData.pilotId,
          deliverableData.ownerId,
      ...
      console.log(
        `Deliverable ${deliverableId} synced to blockchain successfully`
      );
      return null;
    } catch (error) {
...
exports.onRewardPointsUpdate = functions
  .region('us-west1')
  ...
  .onUpdate(async (change, context) => {
    try {
      ...
      const pointsDelta =
        afterData.totalRewardPoints - beforeData.totalRewardPoints;
      ...
      const tx =
        await blockchainManager.contracts.rewardDistribution.recordRewardPointsChange(
          agentId,
          pointsDelta,
      ...
      console.log(
        `Agent ${agentId} reward points update synced to blockchain successfully`
      );
      return null;
    } catch (error) {
...
exports.verifyBlockchainIntegrity = functions
  .region('us-west1')
      ...
      for (const flightDoc of recentFlights.docs) {
        try {
          ...
          const blockchainFlightData =
            await blockchainManager.contracts.flightVerification.getFlight(
              flightId
            );
      ...
      for (const agentDoc of recentAgents.docs) {
        try {
          ...
          const verificationResult =
            await agentRegistrySystem.verifyAgentOnChain(agentId);
