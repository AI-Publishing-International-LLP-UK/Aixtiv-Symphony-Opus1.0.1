/**
 * TeamGateway Implementation
 * Extends BaseGateway to provide authentication for Team services
 */

const BaseGateway = require('./BaseGateway');

/**
 * Gateway for Team authentication and authorization
 */
class TeamGateway extends BaseGateway {
  /**
   * TeamGateway constructor
  ...
  constructor(options = {}) {
    ...
    if (!this.teamService) {
      throw new Error('teamService is required for TeamGateway');
    }
  }
  ...
  async _performAuthentication(context) {
    try {
      ...
      this.logger.error(`Authentication error in TeamGateway: ${error.message}`, { 
        requestId: context.requestId,
        error 
      });
