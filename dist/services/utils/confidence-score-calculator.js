...
export class ConfidenceScoreCalculator {
  ...
  computeWeightedScore(components){
    return this.computeWeightedScoreWithCustomWeights(
      components,
   ...
   */
  computeWeightedScoreWithCustomWeights(
    components,
    weights){
    ...
    if (Math.abs(totalWeight - 1.0) > 0.001) {
      console.warn(
        `Weights do not sum to 1.0 (sum: ${totalWeight}). Normalizing.`
      );
      // Normalize weights
      Object.keys(weights).forEach(key => {
        weights[key ConfidenceWeights] /= totalWeight;
      });
    }

    // Calculate weighted sum
    return Object.keys(components).reduce((score, key) => {
      const typedKey = key ConfidenceScoreComponents;
   ...
   */
  calculateProfessionalAlignment(
    profileData,
    drMatchData){
   ...
   */
  calculateCollaborativeCapacity(
    profileData,
    drMatchData){
    // Sample implementation
    const recommendationsScore = Math.min(
      1.0,
      profileData.recommendations / 10
    );
  ...
  calculateComprehensiveScore(params: {
    ...
    let adjustedScore = Math.min(
      1.0,
      baseScore * (1 + specializationBoost * 0.08)
    );
    ...
    if (params.additionalFactors) {
      ...
      return {
        overallConfidence,
        domainConfidence,
          domainConfidence * (1 + domainAdjustment)
        ),
        profileAuthenticity,
          profileAuthenticity * (1 + authAdjustment)
        ),
      };
    }
  ...
  calculateSpecializationBoost(
    ...
    const alignedSkills = profileData.skills.filter(
      skill =>
    ...
    const skillAlignment = Math.min(
      1.0,
      alignedSkills / Math.max(3, profileData.skills.length / 5)
    );

    // Check job history for relevant experience
    const relevantExperience = profileData.jobHistory.some(
      job =>
