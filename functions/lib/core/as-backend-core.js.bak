...
class UserService {
    ...
    static async createUser(email, password, userData) {
        try {
            ...
            await (0, firestore_1.setDoc)(userDocRef, userDoc);
            ...
            if (userData.entityId &&
                (userData.track === types_1.UserType.CORPORATE ||
                    userData.track === types_1.UserType.ORGANIZATIONAL)) {
                const memberDoc = {
                ...
                await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 'organizations', userData.entityId, 'members', firebaseUser.uid), memberDoc);
            }
    ...
    static async updateUser(id, data) {
        try {
            ...
            await (0, firestore_1.updateDoc)(userDocRef, updateData);
            ...
            const updatedUserDoc = await (0, firestore_1.getDoc)(userDocRef);
     ...
     */
    static async changeUserType(userId, track, position, level) {
        try {
            ...
            if (track || position || level) {
                ...
                // Log the type change
                await this.logUserTypeChange(userId, userData.track, userData.position, userData.level, track || userData.track, position || userData.position, level || userData.level);
            }
            await (0, firestore_1.updateDoc)(userDocRef, updateData);
            // Fetch and return the updated user
            const updatedUserDoc = await (0, firestore_1.getDoc)(userDocRef);
     ...
     */
    static async addSpecializedRole(userId, role) {
        try {
            const userDocRef = (0, firestore_1.doc)(db, 'users', userId);
            const userDoc = await (0, firestore_1.getDoc)(userDocRef);
            if (!userDoc.exists()) {
     ...
     */
    static async createBlockchainVerification(recordType, recordId, blockchainAddress) {
        try {
            ...
            const recordRef = await (0, firestore_1.addDoc)((0, firestore_1.collection)(db, 'blockchainRecords'), blockchainRecord);
            return recordRef.id;
        }
        catch (error) {
     ...
     */
    static async logUserTypeChange(userId, oldTrack, oldPosition, oldLevel, newTrack, newPosition, newLevel) {
        try {
            const logEntry = {
                actorType: 'system',
                actorId: 'typeChange',
            ...
            await (0, firestore_1.addDoc)((0, firestore_1.collection)(db, 'activityLogs'), logEntry);
        }
        catch (error) {
    ...
    static async ensureUserGateways(userId) {
        try {
            ...
            if (needsOwnerGateway) {
                ...
                const querySnapshot = await (0, firestore_1.getDocs)(gatewaysQuery);
                if (querySnapshot.empty) {
...
class AuthService {
    ...
    static async signInWithEmail(email, password) {
        try {
            ...
            await (0, firestore_1.updateDoc)((0, firestore_1.doc)(db, 'users', firebaseUser.uid), {
     ...
     */
    static async logUserActivity(userId, action, status) {
        try {
            const logEntry = {
                actorType: 'user',
                actorId: userId,
...
class OrganizationService {
    ...
    static async createOrganization(data) {
        ...
        try {
            ...
            if ((_a = data.blockchainVerification) === null || _a === void 0 ? void 0 : _a.address) {
            ...
            else if (data.trackType === types_1.UserType.CORPORATE ||
                data.trackType === types_1.UserType.ORGANIZATIONAL) {
                ...
                // Create blockchain verification record
                const txId = await UserService['createBlockchainVerification']('organization', orgId, blockchainAddress);
            ...
            await (0, firestore_1.setDoc)(orgDocRef, orgData);
            // Create appropriate gateways for the organization
            if (data.trackType === types_1.UserType.CORPORATE ||
                data.trackType === types_1.UserType.ORGANIZATIONAL) {
    ...
    static async getOrganizationById(id) {
        try {
            const orgDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 'organizations', id));
            if (!orgDoc.exists()) {
                return null;
            }
            return orgDoc.data();
     ...
     */
    static async addMemberToOrganization(organizationId, userId, role = 'member') {
        var _a;
        try {
            // Check if organization exists
            const orgDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 'organizations', organizationId));
            if (!orgDoc.exists()) {
            ...
            const memberDoc = await (0, firestore_1.getDoc)(memberDocRef);
            if (memberDoc.exists()) {
                // If already a member, update role if needed
                if (((_a = memberDoc.data()) === null || _a === void 0 ? void 0 : _a.role) !== role) {
                    await (0, firestore_1.updateDoc)(memberDocRef, {
                        role,
                        updatedAt: (0, firestore_1.serverTimestamp)(),
                    });
                }
            ...
            // Add member document
            await (0, firestore_1.setDoc)(memberDocRef, {
                userId,
                organizationId,
                role,
    ...
    static async getOrganizationMembers(organizationId) {
        try {
            const membersSnapshot = await (0, firestore_1.getDocs)((0, firestore_1.collection)(db, 'organizations', organizationId, 'members'));
            const members = [];
            for (const memberDoc of membersSnapshot.docs) {
                const memberData = memberDoc.data();
                // Get basic user information
                const userDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 'users', memberData.userId));
                if (userDoc.exists()) {
                    const userData = userDoc.data();
                    members.push({
     ...
     */
    static async createTeam(organizationId, name, description, leaderId) {
        try {
            // Check if organization exists
            const orgDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 'organizations', organizationId));
            if (!orgDoc.exists()) {
            ...
            await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 'teams', teamData.id), teamData);
            // If leader is provided, add them to the team
            if (leaderId) {
                await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 'teams', teamData.id, 'members', leaderId), {
                    userId: leaderId,
                    teamId: teamData.id,
                    role: 'leader',
                    joinedAt: (0, firestore_1.serverTimestamp)(),
     ...
     */
    static async createOrganizationGateways(organizationId, organizationName) {
        try {
...
class AgentService {
    ...
    static async createAgentInstance(data) {
        try {
            ...
            await (0, firestore_1.setDoc)(agentDocRef, agentData);
    ...
    static async getAgentById(id) {
        try {
            const agentDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 'agents', id));
            if (!agentDoc.exists()) {
                return null;
            }
     ...
     * Get agents by owner
     */
    static async getAgentsByOwner(ownerType, ownerId) {
        try {
            ...
            const querySnapshot = await (0, firestore_1.getDocs)(agentsQuery);
     ...
     */
    static async updateAgentInstance(id, data) {
        try {
            const agentDocRef = (0, firestore_1.doc)(db, 'agents', id);
            const agentDoc = await (0, firestore_1.getDoc)(agentDocRef);
            if (!agentDoc.exists()) {
                return null;
            }
            const updateData = Object.assign(Object.assign({}, data), { updatedAt: (0, firestore_1.serverTimestamp)() });
            ...
            await (0, firestore_1.updateDoc)(agentDocRef, updateData);
            // Fetch and return the updated agent
            const updatedAgentDoc = await (0, firestore_1.getDoc)(agentDocRef);
     ...
     */
    static async grantAgentAccess(agentId, accessType, accessId, permissionLevel, grantedBy) {
        try {
            // Check if agent exists
            const agentDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 'agents', agentId));
            if (!agentDoc.exists()) {
            ...
            await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 'agents', agentId, 'access', accessDocId), accessData);
            return true;
        }
        catch (error) {
     ...
     */
    static async createAgentVectorStore(agentId, agentName) {
        try {
            ...
            await (0, firestore_1.setDoc)(vectorStoreRef, vectorStoreData);
     ...
     */
    static async createAgentNFT(agentId, agentName, ownerType, ownerId) {
        try {
            ...
            let ownerAddress = '';
            if (ownerType === 'user') {
                const userDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 'users', ownerId));
                if (userDoc.exists()) {
                    ownerAddress = userDoc.data().blockchainAddress || '';
                }
            }
            else if (ownerType === 'organization') {
                const orgDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 'organizations', ownerId));
            ...
            await (0, firestore_1.updateDoc)((0, firestore_1.doc)(db, 'agents', agentId), {
...
class IntegrationGatewayService {
    ...
    static async createGateway(data) {
        try {
            ...
            await (0, firestore_1.setDoc)(gatewayDocRef, gatewayData);
    ...
    static async getGatewayById(id) {
        try {
            const gatewayDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 'integrationGateways', id));
     ...
     */
    static async getGatewaysByOwner(ownerType, ownerId) {
        try {
     ...
     */
    static async createEndpoint(gatewayId, path, method, description, requiresAuth = true, permissions = [], inputSchema = null, outputSchema = null) {
        try {
            // Check if gateway exists
            const gatewayDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 'integrationGateways', gatewayId));
            if (!gatewayDoc.exists()) {
            ...
            const querySnapshot = await (0, firestore_1.getDocs)(endpointsQuery);
            if (!querySnapshot.empty) {
            ...
            await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 'integrationGateways', gatewayId, 'endpoints', endpointData.id), endpointData);
     ...
     */
    static async generateApiKey(gatewayId, keyName, issuedToType, issuedToId, issuedBy, permissions = [], expiresInDays = 365) {
        try {
            // Check if gateway exists
            const gatewayDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 'integrationGateways', gatewayId));
            if (!gatewayDoc.exists()) {
            ...
            await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 'integrationApiKeys', apiKeyData.id), apiKeyData);
            return {
                keyId: apiKeyData.id,
                apiKey,
                prefix,
     ...
     * Validate an API key
     */
    static async validateApiKey(prefix, apiKey) {
        try {
            ...
            const querySnapshot = await (0, firestore_1.getDocs)(apiKeysQuery);
            if (querySnapshot.empty) {
                return false;
            }
            // Get the stored hash
            ...
            await (0, firestore_1.updateDoc)((0, firestore_1.doc)(db, 'integrationApiKeys', querySnapshot.docs[0].id), {
     ...
     */
    static async createDefaultEndpoints(gatewayId, gatewayType) {
        try {
...
class ActivityLoggerService {
     ...
     * Log an activity
     */
    static async logActivity(actorType, actorId, action, resourceType, resourceId, status = 'success', details, ipAddress, userAgent) {
        try {
            const logEntry = {
                id: (0, uuid_1.v4)(),
                actorType,
            ...
            await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 'activityLogs', logEntry.id), logEntry);
        }
        catch (error) {
     ...
     */
    static async getActivityForResource(resourceType, resourceId, limit = 100) {
        try {
            ...
            const querySnapshot = await (0, firestore_1.getDocs)(logsQuery);
...
class ConversationService {
     ...
     */
    static async createConversation(title, initiatorType, initiatorId, participants) {
        try {
            ...
            await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 'conversations', conversationId), conversationData);
            // Add participants
            const batch = (0, firestore_1.writeBatch)(db);
     ...
     */
    static async addMessage(conversationId, senderType, senderId, content, contentType = 'text', parentMessageId) {
        try {
            ...
            const participantDoc = await (0, firestore_1.getDoc)(participantRef);
            ...
            await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 'conversations', conversationId, 'messages', messageId), messageData);
            // Update conversation's updatedAt timestamp
            await (0, firestore_1.updateDoc)((0, firestore_1.doc)(db, 'conversations', conversationId), {
                updatedAt: now,
            });
     ...
     */
    static async vectorizeMessage(messageId, conversationId, content, senderType, senderId) {
        try {
            ...
            await (0, firestore_1.updateDoc)((0, firestore_1.doc)(db, 'conversations', conversationId, 'messages', messageId), {
                vectorId,
...
class PerformanceMetricsService {
     ...
     */
    static async recordMetric(metricType, subjectType, subjectId, value, unit, metadata) {
        try {
            const metricData = {
                id: (0, uuid_1.v4)(),
                metricType,
            ...
            await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 'performanceMetrics', metricData.id), metricData);
        }
        catch (error) {
     ...
     */
    static async getMetricsForSubject(subjectType, subjectId, metricType, startDate, endDate, limit = 100) {
        try {
            ...
            const querySnapshot = await (0, firestore_1.getDocs)(metricsQuery);
            return querySnapshot.docs
                .map(doc => doc.data())
                .filter(metric => {
                var _a;
...
class S2DOService {
     ...
     */
    static async createS2DOObject(ownerType, ownerId, objectType, data, isPublic = false, encrypt = true) {
        try {
            ...
            if (encrypt) {
                ...
                await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 's2doEncryptionKeys', objectId), {
                    key: encryptionKey,
                    objectId,
                    ownerType,
                    ownerId,
            ...
            await (0, storage_1.uploadBytes)(storageReference, dataBlob);
            // Get the download URL
            const downloadURL = await (0, storage_1.getDownloadURL)(storageReference);
            ...
            await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 's2doObjects', objectId), s2doData);
            return objectId;
        }
        catch (error) {
     ...
     * Get an S2DO object
     */
    static async getS2DOObject(objectId, requesterId) {
        try {
            // Get the object metadata
            const objectDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 's2doObjects', objectId));
            if (!objectDoc.exists()) {
                throw new Error('Object not found');
            }
            const objectData = objectDoc.data();
            // Check access permissions
            const hasAccess = (objectData.ownerType === 'user' &&
            ...
            if (storedData.encrypted) {
                ...
                const keyDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 's2doEncryptionKeys', objectId));
                if (!keyDoc.exists()) {
     ...
     */
    static async grantS2DOAccess(objectId, accessType, accessId, granterId) {
        try {
            // Get the object metadata
            const objectDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 's2doObjects', objectId));
            if (!objectDoc.exists()) {
...
class RaysComputeService {
     ...
     */
    static async submitComputeJob(jobType, requesterId, requesterType, parameters, priority = 'normal') {
        try {
            // Create job document
            ...
            await (0, firestore_1.setDoc)((0, firestore_1.doc)(db, 'raysComputeJobs', jobId), jobData);
    ...
    static async getJobStatus(jobId) {
        try {
            const jobDoc = await (0, firestore_1.getDoc)((0, firestore_1.doc)(db, 'raysComputeJobs', jobId));
            if (!jobDoc.exists()) {
     ...
     */
    static async getJobsForRequester(requesterId, requesterType, status) {
        try {
            ...
            const querySnapshot = await (0, firestore_1.getDocs)(jobsQuery);
            return querySnapshot.docs
                .map(doc => doc.data())
                .sort((a, b) => {
                var _a, _b;
...
//# sourceMappingURL=as-backend-core.js.map