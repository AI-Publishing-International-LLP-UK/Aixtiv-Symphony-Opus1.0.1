...
class BaseIntegrationGateway {
    ...
    setupEndpoints() {
        ...
        this.app.get('/info', async (req, res) => {
            try {
                const gateway = await core_1.IntegrationGatewayService.getGatewayById(this.gatewayId);
                res.status(200).json({
    ...
    async validateApiKey(req, res, next) {
        ...
        try {
            const isValid = await core_1.IntegrationGatewayService.validateApiKey(keyPrefix, apiKey);
            if (!isValid) {
    ...
    // Logging middleware
    async logRequest(req, _res, next) {
        var _a, _b;
        ...
        try {
            // Log the API request
            await core_1.ActivityLoggerService.logActivity(((_a = req.apiKey) === null || _a === void 0 ? void 0 : _a.issuedToType) || 'ANONYMOUS', ((_b = req.apiKey) === null || _b === void 0 ? void 0 : _b.issuedToId) || 'unknown', `API_REQUEST_${req.method}`, 'GATEWAY', this.gatewayId, 'SUCCESS', {
                path: req.path,
                method: req.method,
                query: req.query,
    ...
    async getApiKeyDetails(keyPrefix) {
        const result = await core_1.db.query(`SELECT * FROM integration_api_keys 
...
class OwnerIntegrationGateway extends BaseIntegrationGateway {
    ...
    setupSpecificEndpoints() {
        ...
        this.app.get('/profile', async (req, res) => {
            ...
            try {
                ...
                if (this.ownerType === 'USER') {
                    const result = await core_1.db.query('SELECT * FROM users WHERE id = $1', [
                        this.ownerId,
                    ]);
                    if (result.rows.length === 0) {
                    ...
                    const result = await core_1.db.query('SELECT * FROM organizations WHERE id = $1', [this.ownerId]);
                    if (result.rows.length === 0) {
        ...
        this.app.get('/agents', async (req, res) => {
            try {
                const result = await core_1.db.query(`SELECT ai.*, at.type_code, at.display_name as agent_type_name
        ...
        this.app.get('/solutions', async (req, res) => {
            try {
                const result = await core_1.db.query(`SELECT s.*, ss.subscription_tier, ss.status as subscription_status, 
    ...
    async checkSharedAccess(req, res, next) {
        var _a, _b;
        try {
            ...
            if (resourceType === 'agent') {
                const result = await core_1.db.query(`SELECT * FROM agent_access_controls 
...
class EnterpriseIntegrationGateway extends BaseIntegrationGateway {
    ...
    setupSpecificEndpoints() {
        ...
        this.app.get('/organization', async (_req, res) => {
            try {
                const result = await core_1.db.query('SELECT * FROM organizations WHERE id = $1', [this.organizationId]);
                if (result.rows.length === 0) {
                    return res.status(404).json({ error: 'Organization not found' });
                }
                const organization = {
        ...
        this.app.get('/members', async (_req, res) => {
            try {
                const result = await core_1.db.query(`SELECT om.*, u.username, u.email, u.first_name, u.last_name 
        ...
        this.app.get('/teams', async (_req, res) => {
            try {
                const result = await core_1.db.query('SELECT * FROM teams WHERE organization_id = $1', [this.organizationId]);
        ...
        // Team details
        this.app.get('/teams/:teamId', async (req, res) => {
            try {
                // First, verify that the team belongs to this organization
                const teamResult = await core_1.db.query('SELECT * FROM teams WHERE id = $1 AND organization_id = $2', [req.params.teamId, this.organizationId]);
                if (teamResult.rows.length === 0) {
                    return res.status(404).json({ error: 'Team not found' });
                }
                // Get team members
                const membersResult = await core_1.db.query(`SELECT tm.*, u.username, u.email, u.first_name, u.last_name 
        ...
        this.app.get('/integrations', async (_req, res) => {
            try {
                const result = await core_1.db.query(`SELECT * FROM integration_connections 
        ...
        this.app.post('/webhook/:connectionType', async (req, res) => {
            try {
    ...
    handleSpecificAuthorization(req, res, next) {
        ...
        else if (((_b = req.apiKey) === null || _b === void 0 ? void 0 : _b.issuedToType) === 'ORGANIZATION' &&
            ((_c = req.apiKey) === null || _c === void 0 ? void 0 : _c.issuedToId) === this.organizationId) {
    ...
    async checkOrganizationMembership(req, res, next) {
        var _a;
        try {
            const userId = (_a = req.apiKey) === null || _a === void 0 ? void 0 : _a.issuedToId;
            const result = await core_1.db.query(`SELECT * FROM organization_members 
    ...
    async processWebhook(connectionType, payload) {
        // Log the webhook
        await core_1.ActivityLoggerService.logActivity('EXTERNAL_SERVICE', connectionType, 'WEBHOOK_RECEIVED', 'GATEWAY', this.gatewayId, 'SUCCESS', {
...
class OwnerSubscriberIntegrationGateway extends BaseIntegrationGateway {
    ...
    setupSpecificEndpoints() {
        ...
        this.app.get('/subscribers', async (req, res) => {
            ...
            try {
                ...
                const result = await core_1.db.query(`SELECT ss.*, s.solution_code, s.name as solution_name
        ...
        this.app.get('/subscriptions/:id', async (req, res) => {
            var _a, _b, _c, _d;
            try {
           ...
           WHERE ss.id = $1`, [subscriptionId]);
                ...
                if (
                // Owner can access
                (((_a = req.apiKey) === null || _a === void 0 ? void 0 : _a.issuedToType) === this.ownerType &&
                    ((_b = req.apiKey) === null || _b === void 0 ? void 0 : _b.issuedToId) === this.ownerId) ||
                    // Subscriber can access their own subscription
                    (((_c = req.apiKey) === null || _c === void 0 ? void 0 : _c.issuedToType) === subscription.subscriber_type &&
                        ((_d = req.apiKey) === null || _d === void 0 ? void 0 : _d.issuedToId) === subscription.subscriber_id)) {
                    // Get subscriber details based on type
                    let subscriberDetails = null;
                    if (subscription.subscriber_type === 'USER') {
                        const userResult = await core_1.db.query('SELECT id, username, email, first_name, last_name FROM users WHERE id = $1', [subscription.subscriber_id]);
                        ...
                        const orgResult = await core_1.db.query('SELECT id, name, display_name FROM organizations WHERE id = $1', [subscription.subscriber_id]);
        ...
        this.app.get('/solutions/:solutionCode/access', async (req, res) => {
            var _a, _b, _c, _d;
            try {
                const { solutionCode } = req.params;
                // First, verify that this solution exists
                const solutionResult = await core_1.db.query('SELECT id FROM solutions WHERE solution_code = $1', [solutionCode]);
                if (solutionResult.rows.length === 0) {
                    return res.status(404).json({ error: 'Solution not found' });
                }
                const solutionId = solutionResult.rows[0].id;
                // Check if the requester has an active subscription
                const subscriptionResult = await core_1.db.query(`SELECT * FROM solution_subscriptions 
           WHERE solution_id = $1 
           ...
           AND (end_date IS NULL OR end_date > NOW())`, [solutionId, (_a = req.apiKey) === null || _a === void 0 ? void 0 : _a.issuedToType, (_b = req.apiKey) === null || _b === void 0 ? void 0 : _b.issuedToId]);
                ...
                const accessResult = await core_1.db.query(`SELECT name, features, version FROM solutions WHERE id = $1`, [solutionId]);
        ...
        this.app.post('/data/share', async (req, res) => {
            var _a, _b;
            try {
                ...
                const subscriptionResult = await core_1.db.query(`SELECT ss.id FROM solution_subscriptions ss
           JOIN solutions s ON ss.solution_id = s.id
        ...
        this.app.get('/data/access/:dataId', async (req, res) => {
            try {
    ...
    async checkSubscriptionStatus(req, res, next) {
        ...
        try {
            ...
            const result = await core_1.db.query(`SELECT ss.* FROM solution_subscriptions ss
         WHERE ss.subscriber_type = $1 
         AND ss.subscriber_id = $2
         ...
         AND (ss.end_date IS NULL OR ss.end_date > NOW())`, [
                (_c = req.apiKey) === null || _c === void 0 ? void 0 : _c.issuedToType,
                (_d = req.apiKey) === null || _d === void 0 ? void 0 : _d.issuedToId,
                this.ownerType,
                this.ownerId,
            ]);
            if (result.rows.length === 0) {
...
class IntegrationGatewayFactory {
    static async createGateway(gatewayType, ownerId, ownerType, name, description) {
        try {
            ...
            const gateway = await core_1.IntegrationGatewayService.createGateway({
                gatewayType,
                name,
                description,
                ownerType,
    ...
    static async getExistingGateway(gatewayId) {
        try {
            const gateway = await core_1.IntegrationGatewayService.getGatewayById(gatewayId);
            if (!gateway) {
                return null;
            }
            // Create the appropriate gateway instance
    ...
    static async createAllGatewaysForEntity(ownerType, ownerId, entityName) {
        const gatewayIds = [];
        try {
            // Create owner gateway
            const ownerGateway = await this.createGateway(core_1.GatewayType.OWNER, ownerId, ownerType, `${entityName} Personal Gateway`, `Personal integration gateway for ${entityName}`);
            gatewayIds.push(ownerGateway.gatewayId);
            // Create owner-subscriber gateway
            const subscriberGateway = await this.createGateway(core_1.GatewayType.OWNER_SUBSCRIBER, ownerId, ownerType, `${entityName} Subscriber Gateway`, `Subscriber integration gateway for ${entityName}`);
            gatewayIds.push(subscriberGateway.gatewayId);
            // Create enterprise gateway if organization
            if (ownerType === 'ORGANIZATION') {
                const enterpriseGateway = await this.createGateway(core_1.GatewayType.ENTERPRISE, ownerId, ownerType, `${entityName} Enterprise Gateway`, `Enterprise integration gateway for ${entityName}`);
