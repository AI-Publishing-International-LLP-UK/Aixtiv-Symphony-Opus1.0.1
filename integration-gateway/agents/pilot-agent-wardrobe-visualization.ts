  ...
  /**
   * Comprehensive list of all agent variants
   */
  static getCulturalVariants(): {[key: string]: string} {
    return {
      // Burby Variants
...
function demonstrateVisualizationCenter() {
  ...
  variantsToGreet.forEach(variantName => {
    try {
      const greeting = visualizationManager.generateVisualizationCenterGreeting(
        variantName, 
        variantName.includes('Middle Eastern') ? 'Middle_East' : 
...
export enum EthnicityRepresentation {
...
export interface PhysicalAppearance {
  ...
  bodyType: BodyType;
  ethnicity: EthnicityRepresentation;
...
export interface AgentWardrobe {
  professionalAttire: {
    [DressCode.BUSINESS_FORMAL]: WardrobeItem[];
    [DressCode.BUSINESS_CASUAL]: WardrobeItem[];
    [DressCode.ACADEMIC]: WardrobeItem[];
    [DressCode.TECH_PROFESSIONAL]: WardrobeItem[];
    [DressCode.CULTURAL_TRADITIONAL]: WardrobeItem[];
  };
  accessoires: WardrobeItem[];
  culturalWear: {
    [region: string]: WardrobeItem[];
  };
}

export class AgentVisualizationManager {
  private agentAppearances: Map<string, {
  ...
  private initializeAgentAppearances(): void {
    const baseAgents = [
    ...
    baseAgents.forEach(agent => 
      this.agentAppearances.set(agent.name, {
  ...
  private createCulturalVariants(): void {
    ...
    variants.forEach(variant => 
      this.agentAppearances.set(variant.name, {
   ...
   */
  getAgentAppearance(agentName: string): PhysicalAppearance | undefined {
    return this.agentAppearances.get(agentName)?.appearance;
  }

  /**
   * Get an agent's wardrobe
   */
  getAgentWardrobe(agentName: string): AgentWardrobe | undefined {
    return this.agentAppearances.get(agentName)?.wardrobe;
  }

  /**
   * Select appropriate attire for a specific context
   */
  selectContextualAttire(
    agentName: string, 
    context: string, 
    dressCode: DressCode
   ...
   */
  generateVisualizationCenterGreeting(
    agentName: string, 
    context?: string
  ): {
    greeting: string;
    ...
    const attire = this.selectContextualAttire(
      agentName, 
      context || '', 
      DressCode.BUSINESS_FORMAL
    );
