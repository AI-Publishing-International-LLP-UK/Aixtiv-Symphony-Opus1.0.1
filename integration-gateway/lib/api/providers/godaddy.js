...
async function getDnsRecords(domainName, options = {}) {
  ...
  try {
    ...
    if (error.response && error.response.status === 404) {
      throw new Error(`DNS records not found for domain ${domainName}`);
    }
    throw new Error(`Failed to get DNS records for ${domainName}: ${error.message}`);
  }
}
 ...
 */
async function verifyApiCredentials(options = {}) {
  try {
    const client = createGoDaddyClient({ env: options.env });
    
    // Test credentials by making a simple API call
    await client.get('/domains', { params: { limit: 1 } });
    
    logAgentAction('godaddy-verify-credentials', { success: true });
    return true;
  } catch (error) {
    logAgentAction('godaddy-verify-credentials', { 
      success: false, 
      error: error.message 
    });
    return false;
