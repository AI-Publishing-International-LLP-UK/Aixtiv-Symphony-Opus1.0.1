...
function checkAndFixAdminInitialization() {
  ...
  if (!fs.existsSync(functionsIndexPath)) {
    console.log(chalk.red(`❌ Functions index file not found at ${functionsIndexPath}`));
    return false;
  }
  ...
  if (!content.includes('firebase-admin') && !content.includes('firebase admin')) {
  ...
  if (!CONFIG.adminInitPattern.test(content)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (lines[i].includes('exports.') || lines[i].includes('module.exports')) {
        firstExportIndex = i;
        break;
      }
    }
  ...
  try {
    ...
    Object.entries(CONFIG.requiredDependencies).forEach(([dep, version]) => {
      if (!packageJson.dependencies || !packageJson.dependencies[dep]) {
        ...
        needsUpdate = true;
      } else if (packageJson.dependencies[dep].startsWith('^1') || 
                 packageJson.dependencies[dep].startsWith('^2') || 
                 packageJson.dependencies[dep].startsWith('^3')) {
        console.log(chalk.yellow(`⚠️ Outdated dependency: ${dep} (${packageJson.dependencies[dep]})`));
        packageJson.dependencies[dep] = version;
        needsUpdate = true;
      } else {
...
function checkFirebaseConfig() {
  ...
  if (!fs.existsSync(firebaseJsonPath)) {
    console.log(chalk.red(`❌ Firebase configuration not found at ${firebaseJsonPath}`));
    return false;
  }
  ...
  try {
    ...
    if (!firebaseConfig.functions) {
      console.log(chalk.red('❌ Functions configuration missing in firebase.json'));
      return false;
    }
...
function validateDeployment() {
  ...
  try {
    console.log(chalk.blue('Running Firebase functions:config:get to check credentials...'));
    execSync('firebase functions:config:get', { stdio: 'pipe' });
    console.log(chalk.green('✅ Firebase credentials validated'));
    
    return true;
  } catch (error) {
    console.log(chalk.red(`❌ Firebase credentials validation failed. You may need to run 'firebase login' first.`));
    return false;
  }
}
...
function main() {
  ...
  if (success && validateDeployment()) {
    ...
    console.log(chalk.yellow('Fix the issues above and run this script again.'));
  }
}
