...
var DOQueueHandler = class extends DurableObject {
    ...
    try {
      ...
      response = await this.service.fetch(`${protocol}://${host}${url}`, {
        ...
        headers: {
          ...
          'x-prerender-revalidate': 'caa983facd6f04948da6b9108f45dda8',
          'x-isr': '1',
        },
  ...
  async initState() {
    ...
    this.sql.exec('DELETE FROM failed_state WHERE buildId != ?', 'anb_zu9Nv6VHogYN6Esa_');
    this.sql.exec('DELETE FROM sync WHERE buildId != ?', 'anb_zu9Nv6VHogYN6Esa_');
