...
class S2DOBlockchainService {
     ...
     */
    storeActionVerification(actionId, actionHash, initiatorAddress, verifierAddresses) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // First, check if the action is already recorded
                const actionExists = yield this.actionContract.verifyActionHash(actionId, actionHash);
                if (actionExists) {
                ...
                // Record the action
                const tx = yield this.actionContract.recordAction(actionId, actionType, actionHash, initiatorAddress);
                ...
                for (const verifierAddress of verifierAddresses) {
                    ...
                    // Verify the action
                    const verifyTx = yield this.actionContract.verifyAction(actionId, signature);
                    // Wait for transaction to be mined
                    yield verifyTx.wait();
                    console.log(`Action verified by ${verifierAddress}`);
                }
                // Complete the action
                const completeTx = yield this.actionContract.completeAction(actionId);
                const completeReceipt = yield completeTx.wait();
     ...
     */
    mintNFT(metadata, ownerAddress, contributorAddresses) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b, _c, _d, _e;
            try {
                ...
                // Mint the NFT
                const tx = yield this.nftContract.mintAchievement(ownerAddress, uri, actionId, actionType, initiatorId, contributorIds, contributorShares);
                // Wait for transaction to be mined
                const receipt = yield tx.wait();
     ...
     * @param actionHash Hash of the action data
     */
    verifyActionRecord(actionId, actionHash) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                return yield this.actionContract.verifyActionHash(actionId, actionHash);
            }
            catch (error) {
    ...
    getAllActions() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                return yield this.actionContract.getAllActionIds();
            }
            catch (error) {
    ...
    getAction(actionId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const action = yield this.actionContract.getAction(actionId);
                return {
     ...
     */
    getAchievement(tokenId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const achievement = yield this.nftContract.getAchievement(tokenId);
                return {
    ...
    deploySD20System(adminAddress) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const factoryContract = yield factoryFactory.deploy();
                yield factoryContract.deployed();
                console.log(`Factory contract deployed at ${factoryContract.address}`);
                // Use factory to deploy the system
                const tx = yield factoryContract.deploySD20System(adminAddress);
                const receipt = yield tx.wait();
