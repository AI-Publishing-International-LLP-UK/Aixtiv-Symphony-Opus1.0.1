...
interface DrMatchData {
  // Core profile data matching DrMatchProfile
  userId: string;
  fullName: string;
...
export class UniqueIdGenerator {
  ...
  private readonly NAMESPACE_PREFIX = "dcaf"; // Dream Commander Authentication Framework
  ...
  public generate(data: {
    ...
    drMatchProfile: DrMatchData, // Updated to match naming in main file
   ...
   */
  private createProfileDigest(profileData: ProfileData): string {
   ...
   */
  private createMatchFingerprint(matchData: DrMatchData): string {
    const matchString = JSON.stringify({
      ...
      industries: matchData.industryExperience ? 
        ...
        '',
      // Network insights
      networkStrength: matchData.networkInsights ? matchData.networkInsights.connectionDiversity : 0,
      // Compatibility data
      compatibilityScore: matchData.compatibilityData ? matchData.compatibilityData.domainAlignmentScore : 0
    });
   ...
   */
  private createConfidenceHash(scores: ConfidenceScores): string {
    ...
    const scoresString = JSON.stringify({
      ...
      pAlign: scores.professionalAlignment ? Math.round(scores.professionalAlignment * 100) : 0,
      nDiv: scores.networkDiversity ? Math.round(scores.networkDiversity * 100) : 0,
      cCap: scores.collaborativeCapacity ? Math.round(scores.collaborativeCapacity * 100) : 0,
      aIdx: scores.adaptabilityIndex ? Math.round(scores.adaptabilityIndex * 100) : 0
    });
   ...
   */
  private extractKeyAttributes(profileData: ProfileData): string {
    const keyParts = [
      profileData.userId,
      profileData.name,
      profileData.email || '',
      ...
      profileData.experience ? 
        profileData.experience.length.toString() : 
        '0'
    ];
